...
export function authorizeRoute(user,next){const safe=typeof next==='string'&&next.startsWith('/')&&!next.startsWith('//');return user?{allowed:true}:{allowed:false,redirect:safe?`/login?next=${encodeURIComponent(next)}`:'/login'}}
export function validateTeam(team){const errors={};if(!team.name||team.name.trim().length<2)errors.name='TEAM NAME MUST BE AT LEAST 2 CHARACTERS.';if(!/^[A-Za-z0-9]{2,6}$/.test(team.tag||''))errors.tag='TEAM TAG MUST BE 2–6 LETTERS OR NUMBERS.';if(!['EU WEST','EU NORTH/EAST','EU OTHER'].includes(team.region))errors.region='SELECT A VALID EUROPEAN REGION.';return{ok:Object.keys(errors).length===0,errors}}
