...
export default function RankSection() { const { t }=useTranslation(); const [selected,setSelected]=useState<string | null>(null); return <section id="players" className="editorial-section rank-section">
  ...
  <div className="rank-track" tabIndex={-1}>{rankNames.map((rank,index)=><button type="button" key={rank} className={index===rankNames.length-1?"is-apex":""} onClick={()=>setSelected(rank)} aria-label={`${t("rank.select")} ${rank}`}><span className="rank-badge"><RankEmblem index={index}/></span><b>{rank}</b><small>0{index+1}</small></button>)}</div>
