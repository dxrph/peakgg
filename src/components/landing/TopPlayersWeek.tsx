...
const MOCK = [
  { rank: 1, name: "PhantomX",     wins: 42, elo: 2540 },
  { rank: 2, name: "NightShade",   wins: 38, elo: 2310 },
  { rank: 3, name: "AceViper",     wins: 35, elo: 2180 },
  { rank: 4, name: "BlitzStorm",   wins: 31, elo: 1950 },
  { rank: 5, name: "ShadowReaper", wins: 28, elo: 1820 },
];
    ...
    <section className="py-24 bg-background">
      <div className="container">
        ...
        >
          ...
          <p className="mt-3 text-muted-foreground font-body">The five climbing the fastest right now.</p>
        </motion.div>
        ...
        >
          ...
          {MOCK.map((p, i) => (
            ...
            >
              <span className={`font-display font-bold text-lg flex items-center gap-1 ${rankColor(p.rank)}`}>
                {p.rank <= 3 && <Crown className="h-4 w-4" />}
                {p.rank}
              </span>
        ...
        <div className="text-center mt-6">
          <p className="text-sm text-muted-foreground mb-4 font-body italic">
            You could be here — <Link to="/register" className="text-primary hover:underline">Join now</Link>
          </p>
          <Link to="/leaderboard">
