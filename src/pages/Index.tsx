...
export default function Index() {
  return (
    <div className="pg-site">
      <SEO title="PeakGG — Compete Beyond Ranked" description="Find teammates, enter tournaments, build a team and climb the Peak ranks." path="/" />

      <header className="pg-nav">
        <a href="/" className="pg-brand" aria-label="PeakGG home"><PeakMark small /><span>PEAK<span>GG</span></span></a>
        <nav className="pg-navlinks" aria-label="Primary"><a className="active" href="/">Home</a>{nav.map((item) => <a key={item} href={`/${item.toLowerCase()}`}>{item}</a>)}</nav>
        <div className="pg-nav-actions"><a className="pg-play" href="/register">PLAY NOW</a><button className="pg-lang"><Globe2 size={15}/> EN <ChevronRight size={13}/></button><button className="pg-menu" aria-label="Open menu"><Menu/></button></div>
      </header>
      ...
      <main>
        <section className="pg-hero">
          ...
          <div className="hero-copy">
            ...
            <h1><span>COMPETE</span><span className="red">BEYOND</span><span>RANKED.</span></h1>
        ...
        <section className="pg-tournaments" id="how">
          <div className="section-head"><div><span>COMPETE FOR MORE</span><h2>ACTIVE COMPETITIONS</h2></div><a href="/tournaments">VIEW ALL TOURNAMENTS <ArrowUpRight size={15}/></a></div>
          <div className="tournament-grid">{tournaments.map((t, i) => <article className="t-card t-card-raster" key={t.title}><img className="t-card-image" src={t.image} alt=""/><div className="t-card-shade"/><div className="t-card-copy"><small>{t.eyebrow}</small><h3>{t.title}</h3><p>{t.meta}</p><a href="/tournaments">{t.cta} <ArrowUpRight size={14}/></a></div><span className="card-index">0{i+1}</span></article>)}</div>
        </section>

        <section className="pg-live-grid">
          <div className="panel live-panel"><div className="panel-title"><span>LIVE <b>MATCHES</b></span><a href="/tournaments">VIEW ALL</a></div><div className="feature-match"><div className="team"><div className="team-mark">N</div><b>NOVA CREW</b></div><div className="score"><span>● LIVE</span><strong>13 - 10</strong><small>ASCENT · BEST OF 1</small></div><div className="team"><div className="team-mark alt">V</div><b>VENOM</b></div></div><div className="match-list">{[['ECLIPSE','7 - 9','WOLVES','LIVE'],['REBELS','10 - 13','DYNASTY','LIVE'],['INFERNO','13 - 8','FALCON','15:30'],['UNITY','0 - 0','PHOENIX','17:00']].map(r => <div className="match-row" key={r[0]}><b>{r[0]}</b><strong>{r[1]}</strong><b>{r[2]}</b><span>{r[3]}</span></div>)}</div></div>
          <div className="panel players-panel"><div className="panel-title"><span>TOP <b>PLAYERS</b></span><a href="/leaderboard">VIEW LEADERBOARD</a></div><div className="player-list">{players.map(([rank,name,elo,delta,image]) => <div className="player-row" key={name}><strong>{rank}</strong><div className="player-avatar player-avatar-raster"><img src={image} alt=""/></div><div><b>{name}</b><span>{elo}</span></div><em className={delta.startsWith('-') ? 'down' : ''}>{delta}</em></div>)}</div></div>
        </section>

        <section className="pg-feature-strip"><article><Users/><span>FIND PLAYERS</span><p>Scout serious teammates by role, rank, region and availability.</p><ArrowUpRight/></article><article><Shield/><span>BUILD A TEAM</span><p>Create a roster, recruit players and build your competitive identity.</p><ArrowUpRight/></article><article><Trophy/><span>ENTER CUPS</span><p>Open cups, leagues and community events built for real competition.</p><ArrowUpRight/></article><article><BarChart3/><span>CLIMB ELO</span><p>Every verified result shapes your Peak rating and seasonal position.</p><ArrowUpRight/></article><article><Swords/><span>MAKE A NAME</span><p>Your matches, ranks and results become your competitive profile.</p><ArrowUpRight/></article></section>
