...
import { Loader2, Gavel, ExternalLink, RefreshCw } from "lucide-react";
...
export default function AdminMatches() {
  ...
  const load = async () => {
    ...
    if (matchIds.length) {
      const { data: ms } = await supabase
        .from("matches")
        .select("id,game,kind,status,result_status,player_a_id,player_b_id,score_a,score_b")
        .in("id", matchIds);
    ...
    // Stuck matches: result_status = disputed but NO dispute row
    const { data: dispMatches } = await supabase
      .from("matches")
      .select("id,game,kind,status,result_status,player_a_id,player_b_id,score_a,score_b")
      .eq("result_status", "disputed");
    const knownMatchIds = new Set(matchIds);
    // Need to also pull from full disputes list (not just open) to detect "no row at all"
  ...
  return (
    <AdminLayout
      ...
      description="Open competitive disputes. Resolve from the match page."
      actions={
    ...
    >
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="font-display">Opened</TableHead>
      ...
      {stuck.length > 0 && (
        <div className="rounded-lg border border-warning/30 bg-warning/5 overflow-hidden">
          <div className="px-4 py-3 border-b border-warning/30 text-sm font-display">
